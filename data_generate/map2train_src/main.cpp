...
int main(int argc, char** argv)
{
	...
	if(argc < 2){ 
		cerr << "EMmap -> training data ver 0.4" << endl << endl;
		...
		cerr << "  -sstep [int]         Step number [int] of the sliding voxel def=4" << endl;
	...
	try{
		...
		if(usePdb == true){
		 ...
		 Grid<double> pass(g,contour,rad,Vwindow,vstep,sstep,gnorm,igstart);
		}
	}catch(bad_alloc &e){
