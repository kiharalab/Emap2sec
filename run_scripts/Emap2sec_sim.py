...
def read_csv(recordsFile):
	writefilename = os.path.join("/net/kihara/smaddhur/tensorFlow/", recordsFile + 'ts.tfrecords')
...
def read_csv2(x,y,recordsFile):
	writefilename = os.path.join("/net/kihara/smaddhur/tensorFlow/", recordsFile + '2ts.tfrecords')
	print('before data read')	
	np_x = np.array(x)
	labelarr = np.array(y)
...
def read_records(filename,batch_size,num_epochs):
	...
	readfile = os.path.join("/net/kihara/smaddhur/tensorFlow/", filename + 'ts.tfrecords')
...
def read_records2(filename,batch_size,num_epochs):
	#if not num_epochs: num_epochs = None
	readfile = os.path.join("/net/kihara/smaddhur/tensorFlow/", filename + '2ts.tfrecords')
	#with tf.name_scope('input'):
...
#		y_conv=tf.matmul(h_fc1_drop, W_fc2) + b_fc2
		...
		regularizer3 = lambdA * (tf.nn.l2_loss(W_conv5)+tf.nn.l2_loss(b_conv5))
		cross_entropy = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(y_conv, label)+regularizer3)+tf.reduce_sum(regularizer1)#+tf.reduce_sum(regualaizerl2))# #+ tf.reduce_sum(regularizer2)
		...
		try:
			...
			while not coord.should_stop():
				...
				sess.run(train_step,feed_dict={keep_prob:0.5,lambdA:0.01,beta:10})
				#print(h_conv1.eval(session=sess,feed_dict={keep_prob:0.5,lambdA:0.1}))
				if iter%100 == 0:
					print('Step %d: accuracy = %.2f' % (iter, accuracy.eval(session=sess,feed_dict={keep_prob:0.5,lambdA:0.01,beta:10})))
				iter += 1
...
#			print(np.shape(x_test_arr2[j]))
			fil1 = open('outFile2-L2'+str(j),'w');
			filA = open('outFileAct-L2'+str(j),'w');
...
#			print(sklearn.metrics.confusion_matrix(y_test_arr_reshape, pred_labels))
			...
			y_prob_tlabel2=[]
			y_prob_truelabel2=[]
			for k in test_data2[j][1:len(test_data2[j])]:
				if(int(k[0])==-1):		#check string
					y_prob_mat.append([0,0,0])
#					y_prob[0] = np.vstack([y_prob[0][:index],np.array([0,0,0]),y_prob[0][index:]]) if y_prob[0][:index].size else np.vstack([np.array([0,0,0]),y_prob[0][index:]])
					y_prob_tlabel2.append(3)
					y_prob_truelabel2.append(3)
				else:
					y_prob_mat.append(y_prob[0][index])
					y_prob_tlabel2.append(cum_pred_labels[index])					
					y_prob_truelabel2.append(y_test_arr_reshape[index])					
					index+=1
			print("Done y_prob_mat")
			x_test2 = []
			y_test_2 = []
			#print((y_prob[0]))
			#bkg_count = np.shape(y_prob[0])[0] - (reshaped[j][0]+reshaped[j][1]+reshaped[j][2])
			...
			y_prob_reshape = np.reshape(np.array(y_prob_mat),(reshaped2[j][2],reshaped2[j][1],reshaped2[j][0],3))
			y_prob_tlabel_reshape2 = np.reshape(np.array(y_prob_tlabel2),(reshaped2[j][2],reshaped2[j][1],reshaped2[j][0]))			
			y_prob_truelabel_reshape2 = np.reshape(np.array(y_prob_truelabel2),(reshaped2[j][2],reshaped2[j][1],reshaped2[j][0]))			
			print("Reshape done")
			for a in range(reshaped2[j][2]):
				for b in range(reshaped2[j][1]):
					for c in range(reshaped2[j][0]):				
						...
						y_test_2.append(y_prob_truelabel_reshape2[a][b][c])							
			print(np.shape(np.array(x_test2)))
			print(np.shape(np.array(y_test_2)))
			print("-----------YT-------------")
			#print(y_test_2)
...
#			print(len(y_test_arr2_full[j]))
			...
			fil1 = open('outFile2-L2_new'+str(j),'w');
			fil1.write(filnames2[j])
			for i in range(len(y_test_arr_reshape)):
				fil1.write(str(y_test_arr_reshape[i][0]));
				fil1.write(';');
