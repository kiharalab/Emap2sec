...
j=0
for filname in fil2:
    print("Running Emap2sec Phase1 for dataset : ",filname)
...
for j in range(test_file_count2):
    ...
    fil1.close()
    print("Wrote the output of Phase1 to outputP1_"+str(j))
    y_prob.append(indProb)
    #print("confusion_matrix:")
    #print(sklearn.metrics.confusion_matrix(y_test_arr_reshape, cum_pred_labels)[0:3,0:3])

    print("Running Emap2sec Phase2 for dataset : ",filnames2[j]+"This might take a couple of minutes!")
...
filX = open('./TMP_XFile','w')
filX.write(str(x_test_arr2_full))
filX.close()

filY = open('./TMP_yFile','w')
...
for j in range(test_file_count2):
    ...
    fil1.close()
    print("Wrote the output of Phase2 to outputP2_"+str(j))
